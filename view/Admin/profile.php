...
<div class="container">
    <div class="card px-5">
        <div class="card-body">
            ...
            <div align="center">
            <?php 
                  echo '<img src="../assets/'.$row['image'].'" alt="" width="260" height="260" class="rounded" >';
              ?>
            ...
            <table class="table table-bordered mt-3">
              <tr>
                <td>สถานะ </td>
                <td><?= $row['type'] == 999 ? 'ผู้ดูแลระบบ': '';?></td>
              </tr>
              <tr>
                <td>ชื่อ </td>
            ...
            <center>
              <button class="btn btn-warning " onclick="editProfile()">แก้ไขข้อมูลส่วนตัว</button>
...
<!-- Modal -->
<div class="modal fade" id="ModalProfiles" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">เปลี่ยนชื่อ</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id="resultName">
          <form action="">
            ...
            <div class="mb-2">
              ...
              <input type="text" name="status" id="status" value="<?php echo $row['type'] == 1 ? 'ผู้ดูแลระบบ': '';?>" class="form-control" disabled>
            </div>
            <div class="mb-2">
              <label for="">ชื่อ</label>
              <input type="text" name="fullname" id="fullname" value="<?php echo $row['Teacher_name'] ?>" class="form-control">
            </div>
            <div class="mb-2">
              <label for="">เบอร์โทร</label>
              <input type="text" name="phones" id="phones" maxlength="10" value="<?php echo  $row['phone']?>" class="form-control">
            </div>
            <div class="mb-2">
              <label for="">ชื่อผู้ใช้งาน</label>
              <input type="text" name="users" id="users" value="<?php echo $row['username']?>" class="form-control" disabled>
            </div>
          </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ปิด</button>
        <button type="button" class="btn btn-primary" onclick="updateProfiles()" id="btnName">บันทึก</button>
      </div>
    </div>
  </div>
</div>
